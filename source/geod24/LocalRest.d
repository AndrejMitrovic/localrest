...
private final class LocalScheduler : BaseFiberScheduler
{
    ...
    private class FiberCondition : Condition
    {
        ...
        override bool wait(Duration period) nothrow
        {
            ...
            {
                //writefln("limit: %s", limit);
                //writefln("Time:  %s", this.outer.clock.currTime());
                this.outer.yield();
            }
            return notified;
        }
...
unittest
{
    ...
    node.ctrl.pauseTime();
    node.sleepThenSet(500.msecs, 42);
    Thread.sleep(1.seconds);
    ...
    node.ctrl.pauseTime();
    node.sleepThenSet(500.msecs, 69);
    Thread.sleep(1.seconds);

    // time was stopped, node cannot make progress
    assert(node.getValue() == 42);

    node.ctrl.offsetTime(1.seconds);
    Thread.sleep(100.msecs);

    // time was forcefully increased
    assert(node.getValue() == 69);

    node.sleepThenSet(10.msecs, 420);
